...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, graph_data=None):
        ...
        total_loss = None
        loss_batches = []
        for batch_idx, new_graph_data in enumerate(graph_data):
            self.optimizer.zero_grad()
            losses = loss_func(new_graph_data.to(self.config['device']))
            # logits, con_loss, y, tar_embed, tar_geo_embed, geo_enc_p, sess_enc_p = self.CTR_model(new_graph_data.to(self.config['device']))
    ...
    def _valid_epoch(self, valid_data, val_graph_data=None):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.evaluate(valid_data, val_graph_data)
    ...
    def fit(self, train_data, train_graph_data=None, val_graph_data=None, test_graph_data=None, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        wandb.init(project='MMD', name=f'{nowtime}_{self.config["dataset"]}_{self.config["seed"]}', config=self.config)
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            self.model.pre_epoch_processing()
            train_loss, _ = self._train_epoch(train_data, epoch_idx, graph_data=train_graph_data)
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score, valid_result = self._valid_epoch(test_data, val_graph_data=test_graph_data)
                ...
                # test
                _, test_result = self._valid_epoch(valid_data, val_graph_data=val_graph_data)
                if verbose:
                ...
                if update_flag:
                    ...
                    wandb.log({"recall@5": self.best_test_upon_valid['recall@5'], "recall@10": self.best_test_upon_valid['recall@10'], "recall@20": self.best_test_upon_valid['recall@20']
                               , "ndcg@5": self.best_test_upon_valid['ndcg@5'], "ndcg@10": self.best_test_upon_valid['ndcg@10'], "ndcg@20": self.best_test_upon_valid['ndcg@20']})
    ...
    @torch.no_grad()
    def evaluate(self, eval_data, val_graph_data=None,is_test=False, idx=0):
        r"""Evaluate the model based on the eval data.
        Returns:
        ...
        for batch_idx, (batched_data, new_graph_data) in enumerate(zip(eval_data, val_graph_data)):
            # predict: interaction without item ids
            scores = self.model.full_sort_predict(batched_data, new_graph_data.to(self.config['device']))
